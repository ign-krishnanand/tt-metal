...
class TileSelection(nn.Module):
    ...
    def __init__(self, args, num_cls) -> None:
        super().__init__()
        self.token_size = args.token_size  # the size of the final encoded representations, i.e. number of tiles (4*4)
        self.patch_embed = PatchEmbed(img_size=args.imgsz, patch_size=args.patchsz, in_chans=3)
        ...
        self.fea_mlp3 = Mlp(
            in_features=96 * (2**self.num_layers),
            hidden_features=96 * (2**self.num_layers),
            out_features=96 * (2**self.num_layers),
        )

        self.mask_pre3 = mask_token_inference(dim=96 * (2**self.num_layers), num_heads=1)
        self.mlp_norm3 = nn.LayerNorm(96 * (2**self.num_layers))
        self.mlp3 = Mlp(in_features=96 * (2**self.num_layers), hidden_features=96, out_features=96)
        self.linear3 = nn.Linear(96, num_cls)
        ...
        self.mask_pre2 = mask_token_inference(dim=96 * (2**self.num_layers), num_heads=1)
        self.mlp_norm2 = nn.LayerNorm(96 * (2**self.num_layers))
        self.mlp2 = Mlp(in_features=96 * (2**self.num_layers), hidden_features=96, out_features=96)
        self.linear2 = nn.Linear(96, num_cls)
        ...
        self.mask_pre1 = mask_token_inference(dim=96 * (2**self.num_layers), num_heads=1)
        self.mlp_norm1 = nn.LayerNorm(96 * (2**self.num_layers))
        self.mlp1 = Mlp(in_features=96 * (2**self.num_layers), hidden_features=96, out_features=96)
        self.linear1 = nn.Linear(96, num_cls)
    ...
    def forward(self, x):
        ...
        # predict 4*4 representations, for 64*64 tile
        fea_3 = torch.cat((mask_tokens, self.fea_mlp3(x3)), dim=1)

        mask_tokens = fea_3[:, 0, :].unsqueeze(1)
        mask_3 = self.mask_pre3(fea_3)  # [B, 16, 96*32]
        mask_3 = self.mlp3(self.mlp_norm3(mask_3))
        mask_3 = self.linear3(mask_3)
        B, N, C = mask_3.shape
        mask_3 = mask_3.transpose(1, 2).reshape(B, C, self.token_size, self.token_size)

        # predict 8*8 representations, for 32*32 tile
        ...
        mask_2 = mask_2.transpose(1, 2).reshape(B, C, self.token_size * 2, self.token_size * 2)

        # predict 16*16 representations, for 16*16 tile
        ...
        mask_1 = mask_1.transpose(1, 2).reshape(B, C, self.token_size * 4, self.token_size * 4)
