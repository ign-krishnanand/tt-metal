...
def test_rhag(
    ...
    logger.info(f"Residual connection: {resi_connection}")
...
@pytest.mark.parametrize("device_params", [{"l1_small_size": 32768}], indirect=True)
def test_rhag_memory_config(device):
    ...
    does_pass_dram, pcc_message_dram = comp_pcc(ref_output, tt_torch_output_dram, 0.85)
