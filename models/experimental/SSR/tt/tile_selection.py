...
class TTTileSelection(LightweightModule):  
    def __init__(self, device, parameters, args, num_cls, memory_config=None):  
        ...
        self.mask_pre3 = TTMaskTokenInference(  
            device=device,  
            parameters=parameters["mask_pre3"],  
            dim=final_dim,  
            num_heads=1  
        )  
        ...
        self.mask_pre2 = TTMaskTokenInference(  
            device=device,  
            parameters=parameters["mask_pre2"],  
            dim=final_dim,  
            num_heads=1  
        )  
        ...
        self.mask_pre1 = TTMaskTokenInference(  
            device=device,  
            parameters=parameters["mask_pre1"],  
            dim=final_dim,  
            num_heads=1  
        )  
    ...
    def __call__(self, x):  
        ...
        # mask_tokens = ttnn.repeat(self.mask_token_weight, [B, 1, 1])  
        mask_tokens = ttnn.reshape(self.mask_token_weight, [B, 1, self.mask_token_weight.shape[-1]])
          
        # Process scale 3 (finest scale)  
        fea_3_processed = self.fea_mlp3(x3) 
        print("##################################") 
        print(mask_tokens.shape, fea_3_processed.shape) 
        print("##################################") 
        fea_3 = ttnn.concat([mask_tokens, fea_3_processed], dim=1)  
